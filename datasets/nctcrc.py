...
class NCTCRCDataset(BaseDataset):
    def _get_labels(self) -> pd.DataFrame:
        ...
        if self.split == 'nov_path':
            df = pd.read_csv(csv_path)[
                ["image_path"] + self.configs.dataset.labels_ood
            ]
            df = df[df[self.configs.dataset.labels_ood].sum(axis=1) >= 1]
        else:
            df = pd.read_csv(csv_path)[
                ["image_path"] + self.configs.dataset.labels
            ]
    ...
    def __getitem__(self, index: int) -> Any:
        ...
        image_path = self.images_dir / f"{row['image_path']}.png"
